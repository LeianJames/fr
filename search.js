...
// Function to navigate to book location
function navigateToBookLocation(book) {
  // Map categories to their respective page URLs
  const categoryUrls = {
    'Academics': 'Academics Books/academics.html',
  ...
  const categoryUrl = categoryUrls[book.category];
 
  if (categoryUrl) {
    // Navigate to the category page
    window.location.href = categoryUrl;
  } else {
    // If category URL is not defined, just display the book details
    displayResults([book]);
  }
}
...
function displayResults(results) {
  ...
  results.forEach(book => {
    ...
    // Add click event to navigate to book location
    listItem.addEventListener('click', () => {
      navigateToBookLocation(book);
    });
...
function displaySuggestions(query) {
  ...
  suggestionsToShow.forEach(title => {
    ...
    listItem.addEventListener('click', () => {
      ...
      // If we found exactly one book, navigate to its location
      if (results.length === 1) {
        navigateToBookLocation(results[0]);
      } else {
        // Otherwise just display all matching results
